    ...
    const handleHomeAPI = async () => {
        ...
        if(zipCode !== null) {
            ...
            const response = await fetch(url, {
                ...
                headers: {
                    'X-RapidAPI-Key': '838521e75amshd5d352c5b5020b1p1b3e69jsn39226c67a128',
            ...
            for(var i = 0; i < houses.data.length; i++) {
                const latLong = {lat: houses.data[i].location.latitude, lng: houses.data[i].location.longitude};
                ...
                <div>
                    <div className="location-container" style={{ textAlign: "center" }}>
                        ...
                        <center>
                            ...
                            <button className = "sideBar-button" onClick={handleHomeAPI} > Get Home List </button>
                            <button className = "sideBar-button" onClick={handleCrimeMap} > Crimes Near </button>
                            <button className = "sideBar-button" onClick={handleSafestHouseClick} > Get Safest House </button>
                        </center>
                    </div>
                </div>   
                <HomeMap crimes = {crimes} houseList = {houseList} mapContainerStyle = {mapContainerStyle} onLoad= {onLoad}  center={center} office= {office}/>
